...
public class Luban {
    ...
    private File thirdCompress(@NonNull File file) {
        String thumb = mCacheDir.getAbsolutePath() + "/" + FileUtils.createName(file.getAbsolutePath());
        ...
        if (scale <= 1 && scale > 0.5625) {
            if (height < 1664) {
                if (file.length() / 1024 < 150)
                    return saveImage(thumb, BitmapFactory.decodeFile(filePath), -1);
    ...
    private File firstCompress(@NonNull File file) {
        ...
        String filePath = file.getAbsolutePath();
        String thumbFilePath = mCacheDir.getAbsolutePath() + "/" + FileUtils.getName(filePath);
