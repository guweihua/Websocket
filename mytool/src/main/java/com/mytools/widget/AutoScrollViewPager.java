...
public class AutoScrollViewPager extends ViewPager {
        ...
        if (slideBorderMode == SLIDE_BORDER_MODE_TO_PARENT || slideBorderMode == SLIDE_BORDER_MODE_CYCLE) {
            ...
            if ((currentItem == 0 && downX <= touchX) || (currentItem == pageCount - 1 && downX >= touchX)) {
                if (slideBorderMode == SLIDE_BORDER_MODE_TO_PARENT) {
                    ...
                    getParent().requestDisallowInterceptTouchEvent(true);//阻止父层获事件
    ...
    /**
     * netInstance auto scroll time in milliseconds, default is {@link #DEFAULT_INTERVAL}
     *
     * @return the interval
     */
    ...
    /**
     * netInstance auto scroll direction
     *
    ...
    /**
     * netInstance how to process when sliding at the last or first item
     *
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                float horizontallGap = Math.abs(evX - oldX);
                float verticalGap = Math.abs(evY - oldY);


                /** 说明:
                 *如果是横向移动,就让父控件重新获得触摸事件
                 */
                if (verticalGap > horizontallGap) {
                    getParent().requestDisallowInterceptTouchEvent(false);
                } else {
                    getParent().requestDisallowInterceptTouchEvent(true);
                }
